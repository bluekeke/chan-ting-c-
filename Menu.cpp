...
Order::Order(string dishName, string dishID, double price, string customerName, string adress, string phone, Date date) :Menu(dishName, dishID, price), bookData(date)
{
...
const bool Order::operator==(Order order2)
{
	...
	if (!(order2.bookData == bookData)) return false;
	return true;
}
